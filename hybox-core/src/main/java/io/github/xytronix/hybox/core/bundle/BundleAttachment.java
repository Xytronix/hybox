...
public record BundleAttachment(String pathInZip, byte[] data) {
    ...
    public boolean isText() {
        int dot = pathInZip.lastIndexOf('.');
        return dot >= 0 && java.util.Set.of(".txt", ".json", ".html", ".log", ".properties",
            ".xml", ".yaml", ".yml", ".cfg", ".csv")
            .contains(pathInZip.substring(dot).toLowerCase(java.util.Locale.ROOT));
    }
}
